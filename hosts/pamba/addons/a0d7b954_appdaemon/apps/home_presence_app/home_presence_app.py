...
class HomePresenceApp(ad.ADBase):
    ...
    def presence_message(self, event_name, data, kwargs):
        ...
        if not topic.startswith(self.monitor_topic):
            return
        
        payload = data.get("payload")
        self.adapi.log(f"{topic} payload: {payload}", level="DEBUG")
#        self.adapi.log(f"****Line {self.lnn()}: Presence topic {topic} *** with payload {payload}")   #tongai
        ...
        self.adapi.log("line 396 checking {!r} sensor set up".format(device_conf_sensor), level="DEBUG")
    ...
    def confidence_updated(self, entity, attribute, old, new, kwargs):
        ...
        device_conf_sensors = self.home_state_entities.get(device_entity_id)
   #     self.adapi.log(f"Line {self.lnn()}: Updating Conf sensors *** entity {device_entity_id} and state {device_state_sensor_value}")   #tongai
        if device_conf_sensors is None:
            self.adapi.log(
                f"Got Confidence Value for {device_entity_id} but device"
                " is not set up (no sensors found).",
  ...
  #      self.adapi.log(f"Line {self.lnn()}: sensor: {sensor} updated to {state}\n and attributes: {attributes}")   #tongai
    ...
    def run_location_clean(self, kwargs):
        ...
        for sensor in monitor_sensors:
            ...
            if len(sens) == 0:
                ...
                self.adapi.log(f"Removing sensor {sensor}", level="WARNING")
   ...
   #     self.adapi.log(f"Line {self.lnn()}: and appdaemon sensor is {appdaemon_conf_sensor}")   #tongai
    ...
    def load_known_devices(self, kwargs):
        ...
        if self.args.get("known_devices") is not None:
            for device in self.args["known_devices"]:
                ...
                timer += 3
    #        self.adapi.log(f"Line {self.lnn()}: load_known_devices conplete")   #tongai
...
# tongai code
#        self.adapi.log(f"Line {self.lnn()}:Topic is: {mq_msg[0]} and payload is: {mq_msg[1]}", level="DEBUG")
#
    def lnn(self):
